    ...
    <Layout
      ...
      rightContent={
        <div className="flex items-center px-2 border -mt-6 rounded-md">
          <input type="text" placeholder="Search" className="outline-none py-1 px-3 text-[14px] text-gray-400" />
        </div>
      }
    >
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mt-2">
        {loading ? (
          Array.from({ length: 5 }).map((_, index) => (
            <div key={index} className="bg-gray-200 dark:bg-gray-700 p-4 rounded-lg">
          ...
          deals.map((deal) => (
            <div key={deal.id} className="border border-gray-200 dark:border-gray-600 rounded-lg">
              <img
              ...
              />
              <div className="p-4">
                <h2 className="text-lg font-semibold text-primary">Project {deal.project}</h2>
                <p className="text-sm text-gray-500 dark:text-gray-400">Sector: {deal.sector}</p>
                <p className="text-sm text-gray-500 dark:text-gray-400">Country: {deal.country}</p>
                <p className="text-sm text-gray-500 dark:text-gray-400">EBITDA: {deal.ebitda}</p>
                <p className="text-sm text-gray-500 dark:text-gray-400">Total Assets: {deal.total_assets}</p>
                <p className="text-sm text-gray-500 dark:text-gray-400">Deal Type: {deal.deal_type}</p>
                <p className="text-sm text-gray-500 dark:text-gray-400">Ticket Size: {deal.ticket_size}</p>
                <p className="text-sm text-gray-500 dark:text-gray-400">Use of Funds: {deal.use_of_funds}</p>
              </div>
