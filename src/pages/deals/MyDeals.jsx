...
const MyDeals = () => {
  ...
  const handleConfirmWithdraw = () => {
    ...
    setTimeout(() => {
      setDeals((prevDeals) => prevDeals.filter((d) => d.id !== selectedDeal.id));
  ...
  const renderShimmer = () =>
    Array.from({ length: 5 }).map((_, index) => (
      <tr key={index} className="animate-pulse rounded-lg bg-gray-200 dark:bg-gray-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center text-center p-4 bg-gray-100 dark:bg-gray-700 rounded-lg">
            ...
            <p className="text-lg text-gray-700 dark:text-gray-300">No Deals Available</p>
            <p className="text-sm text-gray-500 dark:text-gray-400">You currently don't have any deals.</p>
          </div>
        ) : (
          deals.map((deal) => (
            <div
