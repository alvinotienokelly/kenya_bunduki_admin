...
import { getDealsByUserPreference } from '../../services/api_service';
...
const MyDeals = () => {
  ...
  const getDeals = async () => {
    try {
      const response = await getDealsByUserPreference();
    ...
    >
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
        {loading ? (
          ...
          deals.map((deal) => (
            <div key={deal.id} className="border w-full pr-4 justify-between border-gray-200 dark:border-gray-600 flex items-center rounded-lg">
              <div className="flex items-center gap-4">
                ...
                <div className="flex flex-col">
                  <h2 className="text-lg px-4 font-semibold">Name: {deal.project}</h2>
                  <p className="text-sm px-4 text-gray-500 dark:text-gray-400">Deal Size: {deal.deal_size}</p>
                  <p className="text-sm px-4 text-gray-500 dark:text-gray-400">Region: {deal?.region}</p>
                  <p className="text-sm px-4 text-gray-500 dark:text-gray-400">Sector: {deal?.sector}</p>
                </div>
              </div>
              <div className="w-[240px] flex items-center gap-4">
                <button className='bg-primary text-[13px] text-white px-2 py-1 w-1/2 rounded'>
                  Withdraw
                </button>
